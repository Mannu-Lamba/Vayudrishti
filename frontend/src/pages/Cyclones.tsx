...
export default function Cyclones() {
  ...
  const strongest = useMemo(() => [...cyclones].sort((a, b) => (b.peakWindKmh ?? b.windKmh ?? -1) - (a.peakWindKmh ?? a.windKmh ?? -1))[0], [cyclones]);
  ...
  if (regionalQuery.loading || regionsQuery.loading) registry = <div className="panel-body"><LoadingBlock label="LOADING CYCLONE DATA" rows={4} /></div>;
  ...
  else if (!cyclones.length) registry = <div className="panel-body"><StateNotice variant="no-systems" message={`The database holds no storm for the ${details.label}.`} /></div>;
    ...
    <button type="button" key={cyclone.id} onClick={() => selectCyclone(cyclone)} aria-pressed={selected?.id === cyclone.id} className={`cyclone-list-item ${selected?.id === cyclone.id ? "cyclone-list-item-active" : ""}`} data-testid={`cyclone-list-item-${cyclone.id}`}>
      <div className="list-item-code"><span className="category-dot" style={{ background: categoryColor(cyclone.peakCategory ?? cyclone.category) }} />{cyclone.code}<span className="list-item-status">{cyclone.status}</span></div>
      <strong>{cyclone.name}{cyclone.season ? ` (${cyclone.season})` : ""}</strong>
      <span className="list-item-meta"><MapPin size={12} />peak {cyclone.peakCategory ?? "category unknown"} · {cyclone.region} · {formatLatitude(cyclone.location.latitude)}</span>
      <div className="list-item-wind"><Wind size={13} />{formatWind(cyclone.peakWindKmh ?? cyclone.windKmh, preferences.windUnit)}<ChevronRight size={14} /></div>
    </button>
  ))}</div>;

  return <div className="page-stack" data-testid="cyclones-page">
    <div className="page-intro-row">
      <div><div className="section-kicker"><LocateFixed size={13} /> STORM REGISTRY / {padCount(cyclones.length)} STORMS</div><h2 className="page-heading">Cyclone monitor</h2><p className="page-subheading">The most recent storms in cyclone_database, their tracks and motion — {details.path}.</p></div>
    ...
    <div className="monitor-grid">
      <Panel eyebrow="STORM REGISTRY · CYCLONE_DATABASE" title={`Storms in ${details.label}`} data-testid="cyclone-registry-panel">
        <div className="registry-stats" data-testid="registry-stats">
          ...
          <div><small>Strongest</small><strong data-testid="registry-stat-strongest">{strongest ? `${strongest.name} · ${formatWind(strongest.peakWindKmh ?? strongest.windKmh, preferences.windUnit)}` : "—"}</strong></div>
