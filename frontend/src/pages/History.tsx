...
export default function History() {
  ...
  return <div className="page-stack" data-testid="history-page">
    ...
    <div className="history-top-grid">
      ...
      <Panel eyebrow="REFERENCE WINDOW" title="Archive coverage" data-testid="archive-coverage-panel"><div className="archive-stat"><span className="archive-stat-value">{summary.data?.indexedSystems ?? "—"}</span><span>storms indexed</span></div><div className="archive-lines"><span><CalendarDays size={14} /> {range}</span><span><MapPin size={14} /> {summary.data?.coverage ?? "—"}</span><span><Database size={14} /> Served by the archive API</span></div></Panel>
    </div>
      ...
      {archive.loading ? <div className="panel-body"><LoadingBlock label="LOADING ARCHIVE" rows={4} /></div>
        : archive.error ? <div className="panel-body"><ApiErrorState error={archive.error} onRetry={archive.refetch} subject="the archive" /></div>
          : !records.length ? <div className="panel-body"><StateNotice variant="empty" title="NO HISTORICAL RECORDS" message="No archived systems match this filter." /></div>
            : <div className="history-table-wrap"><table className="history-table"><thead><tr><th>System</th><th>Season</th><th>Area / basin</th><th>Peak wind</th><th>Observed</th><th>Record</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.name}</strong><span>{record.category ?? "Category unknown"}</span></td><td>{record.year}</td><td>{record.area} · {record.basin}</td><td>{record.peakWindKmh != null ? `${record.peakWindKmh} km/h` : "—"}</td><td>{record.firstObservedAt ? `${formatObservedAt(record.firstObservedAt).split(" · ")[0]} – ${formatObservedAt(record.lastObservedAt).split(" · ")[0]}` : "—"}</td><td><button type="button" className="table-action" data-testid={`history-record-${record.id}-button`}><ChevronRight size={15} /></button></td></tr>)}</tbody></table></div>}
